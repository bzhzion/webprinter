# 🖨️ WebPrinter — Tiny PDF Printing & Scanning
...
### Optional webhook
- Pass `webhook_url` (any `http(s)://` URL) and WebPrinter `POST`s a JSON payload to it once the scan finishes — `{"job_id": "...", "status": "done|failed", "message": "...", "download_url": "..."}`
- Signed with header `X-WebPrinter-Signature: sha256=<hmac>` (HMAC-SHA256 of the raw JSON body, keyed with `api_token`) so the receiver can verify it came from this instance
...
## 🐳 Docker Usage
- Image: `ghcr.io/bzhzion/webprinter` (multi-arch: `linux/amd64`, `linux/arm64`, `linux/arm/v6`)
...
### Run Examples
- Linux/macOS:
  - `docker run -d --name webprinter -p 8081:80 --restart unless-stopped ghcr.io/bzhzion/webprinter:latest`
  - With env: `docker run -d --name webprinter -p 8081:80 --restart unless-stopped -e PRINTER_NAME=DeskJet_3630 -e CUPS_SERVER=host.docker.internal -e CUPS_PORT=631 -e API_TOKEN=CHANGE_ME_SECRET_TOKEN -e MAX_FILE_SIZE_MB=20 -e ALLOWED_MIME_TYPES=application/pdf ghcr.io/bzhzion/webprinter:latest`
  - Multiple printers: `docker run -d --name webprinter -p 8081:80 --restart unless-stopped -e PRINTERS=DeskJet_3630,OfficeLaser -e CUPS_SERVER=host.docker.internal -e CUPS_PORT=631 -e API_TOKEN=CHANGE_ME_SECRET_TOKEN ghcr.io/bzhzion/webprinter:latest`
  - Protect UI: `docker run -d --name webprinter -p 8081:80 --restart unless-stopped -e INDEX_PASSWORD=MySecret ghcr.io/bzhzion/webprinter:latest`
- Windows PowerShell:
  - `docker run -d --name webprinter -p 8081:80 --restart unless-stopped ghcr.io/bzhzion/webprinter:latest`
  - With env: `docker run -d --name webprinter -p 8081:80 --restart unless-stopped -e PRINTER_NAME=DeskJet_3630 -e CUPS_SERVER=host.docker.internal -e CUPS_PORT=631 -e API_TOKEN=CHANGE_ME_SECRET_TOKEN -e MAX_FILE_SIZE_MB=20 -e ALLOWED_MIME_TYPES=application/pdf ghcr.io/bzhzion/webprinter:latest`
  - Multiple printers: `docker run -d --name webprinter -p 8081:80 --restart unless-stopped -e PRINTERS=DeskJet_3630,OfficeLaser -e CUPS_SERVER=host.docker.internal -e CUPS_PORT=631 -e API_TOKEN=CHANGE_ME_SECRET_TOKEN ghcr.io/bzhzion/webprinter:latest`
...
- With scanners: `docker run -d --name webprinter -p 8081:80 --restart unless-stopped -e SCANNERS=DeskJet_3630=http://192.168.1.50/eSCL ghcr.io/bzhzion/webprinter:latest`
...
- WebPrinter — créé par Painteau. Contribuez sur GitHub: https://github.com/bzhzion/webprinter
